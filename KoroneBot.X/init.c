...
void initRS232(void) //baud=9600, 8bits donn�es, pas de bit de parit�, un bit de stop, on ne fait que transmettre
{
...
void initPWM(void){
    ...
    TRISAbits.RA6=0; //les bits sont � d�faut � 0, mais c'est un rappel: il ne faut pas oublier que RA6 et RA7 sont utilis�s pour indiquer le sens de rotation des moteurs
    TRISAbits.RA7=0;
...
void initADC(void){
    ...
    ADCON2bits.ACQT = 3;
    ADCON2bits.ADCS = 4;
    ADCON2bits.ADFM = 0;//on justifie � gauche, on ignore les deux bits de poids faible
